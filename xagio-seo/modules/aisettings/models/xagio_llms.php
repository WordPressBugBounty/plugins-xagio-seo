...
if (!class_exists('XAGIO_MODEL_LLMS')) {
    ...
    class XAGIO_MODEL_LLMS
    {
        ...
        public static function generateMarkdown($cfg = null)
        {
            ...
            // Group content per the llms.txt structure (doc 2.5): core service pages
            // first, then any non-page types (e.g. blog Posts), and finally secondary
            // pages (company + policies: contact, about, privacy, terms) under "## Optional".
            $service_items  = [];
            $optional_items = [];
            $type_sections  = []; // heading => items (non-page post types)
            ...
            foreach ($post_types as $post_type) {
                ...
                if ($post_type === 'page') {
                    foreach ($items as $item) {
                        if (self::classifyGroup($item['title'], $item['url']) === 'services') {
                            $service_items[] = $item;
                        } else {
                            $optional_items[] = $item;
                        }
                    ...
                    $type_sections[self::postTypeHeading($post_type)] = $items;
                }
            }
            ...
            if (!empty($service_items)) {
                ...
                foreach ($service_items as $item) {
                    $lines[] = self::itemRow($item, true);
                }
                $lines[] = '';
            }

            foreach ($type_sections as $heading => $items) {
                $lines[] = '## ' . $heading;
                foreach ($items as $item) {
                    $lines[] = self::itemRow($item, true);
                }
            ...
            // Link the Open Knowledge Format bundle so agents can discover the
            // machine-readable knowledge base from llms.txt.
            if (get_option('XAGIO_OKF_ENABLED') === '1') {
                $lines[] = '## Knowledge Bundle';
                $lines[] = '- [Open Knowledge Format bundle](' . esc_url(home_url('/okf/')) . '): machine-readable knowledge base for AI agents.';
                $lines[] = '';
            }

            // Secondary pages (contact/about + policies) grouped last, as terse links.
            if (!empty($optional_items)) {
                $lines[] = '## Optional';
                foreach ($optional_items as $item) {
                    $lines[] = self::itemRow($item, false);
                }
                $lines[] = '';
            }
        ...
        /**
         * Classify a page into 'services' (core) or a secondary bucket, from a slug/title
         * keyword heuristic (mirrors the OKF index grouping). Company + policy pages both
         * land outside 'services' so they can be grouped under "## Optional".
         */
        private static function classifyGroup($title, $url)
        {
        ...
        private static function itemRow($item, $with_desc = true)
        {
            $title = $item['title'] !== '' ? $item['title'] : $item['url'];
            $row   = '- [' . self::escapeMarkdownText($title) . '](' . $item['url'] . ')';
            if ($with_desc && $item['description'] !== '') {
        ...
        public static function saveLlmsSettings()
        {
            ...
            // The OKF index links back to /llms.txt. Its bundle is cached (unlike the
            // live-generated llms.txt), so rebuild it now to reflect the current state.
            if (class_exists('XAGIO_MODEL_OKF')
                && get_option('XAGIO_OKF_ENABLED') === '1'
                && method_exists('XAGIO_MODEL_OKF', 'rebuildBundle')) {
                XAGIO_MODEL_OKF::rebuildBundle();
            }
